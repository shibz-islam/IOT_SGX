...
std::string base_topic_name = "iot/utd/data/";
...
bool IoTMQTTWrapper::publishMessage(const char *topic, const char *msg) {
    std::string topic_name = base_topic_name + std::string(topic);
    int rc = publish(NULL, topic_name.c_str(), strlen(msg), msg);
    if(rc==0){
