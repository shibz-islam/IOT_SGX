...
sgx_status_t encryptMessageAES_struct(Rule *decMessageIn, size_t len, char *encMessageOut, size_t lenOut, char *tagMessageIn){
    printf("Started Encryption Test.....");
    //printf("\n### Message to encrypt: %s\n", decMessageIn);
    //printf("###MessageLen:%d, encMessageLen:%d\n",len, lenOut);
    sgx_aes_gcm_128bit_key_t *key = (sgx_aes_gcm_128bit_key_t*)gcm_key;
    uint8_t *iv = (uint8_t *)gcm_iv;
    ...
    uint8_t *buffer=(uint8_t*)malloc(sizeof(Rule));
    memcpy(buffer,(uint8_t*)&decMessageIn, sizeof(Rule));

    uint8_t p_dst[lenOut] = {0};
    uint8_t p_dst2[16] = {0};

    sgx_status_t stat = sgx_rijndael128GCM_encrypt(key, buffer, len, p_dst, iv, SGX_AESGCM_IV_SIZE, aad, SGX_AESGCM_MAC_SIZE, (sgx_aes_gcm_128bit_tag_t *) p_dst2);
    check_error_code(stat);
    if(stat == 0){
        memcpy(encMessageOut, p_dst, lenOut);
        memcpy(tagMessageIn, p_dst2, SGX_AESGCM_MAC_SIZE);
...
sgx_status_t decryptMessageAES_struct(char *encMessageIn, size_t len, Rule *decMessageOut, size_t lenOut, char *tag){
    /* not working */
    printf("Started Decryption.....");
    //printf("\n### Data, tag: \n %s\n %s\n", encMessageIn, tag);
    //printf("### Data, tag sizes: \n %ld\n %ld\n", len, strlen(tag));
    ...
    check_error_code(stat);
    if(stat == 0){
        memcpy(&decMessageOut, (char*)&p_dst, lenOut);
        //memcpy(decMessageOut, p_dst, lenOut);
        //decMessageOut[lenOut] = '\0';
        //printf("### Decrypted message: %s with length %ld\n", decMessageOut, strlen(decMessageOut));
    }else{
